...
class TestRedisPipeline < Test::Unit::TestCase
  ...
  def test_pipeline_has_empty_command_list
    assert_equal 0, @pipeline.commands.length
  end
  ...
  def test_add_commands_adds_array_of_commands_as_seperate_commands
    ...
    assert_equal commands.length, @pipeline.commands.length
  end

  def test_add_commands_adds_string_as_single_command
    commands = "hset person:0 first_name joe"
    @pipeline.commands.add(commands)
    assert_equal 1, @pipeline.commands.length
  end

  def test_commands_adds_aliasing
    commands = "hset person:0 first_name joe"
    ...
    commands = ["hset person:0 first_name joe", "hest person:0 last_name smith"]
    @pipeline.commands << commands
    assert_equal 3, @pipeline.commands.length
  end
  ...
  def test_add_commands_queues_commands_at_end
    ...
    assert_equal commands[0], @pipeline.commands.first
    assert_equal last_command, @pipeline.commands.last
  end

   # commands.batch is now private. I'm not sure if these tests need to be rewritten or if their logic is now not needed.
   ...
   #   single_batch = @pipeline.commands.batch
   #   
   ...
   #   assert_equal (count - @pipeline.commands.batch_size), @pipeline.commands.length
   # end
   ...
   def test_execute_sends_commands_to_redis
    ...
    first_command = @pipeline.commands.first.split(" ")
    last_command = @pipeline.commands.last.split(" ")
  ...
  def test_after_execute_no_items_in_command
    @pipeline.commands.add(three_batches_of_commands)
    @pipeline.execute
    assert_equal 0, @pipeline.commands.count
  end
  ...
  private
    ...
    def three_batches_of_commands
      ...
      (0..number_of_commands).each do |i|
        ...
        commands << "hset person:#{i} last_name #{last}" 
